...
async function fetchDetail() {
  try {
    const [postRes, commentRes] = await Promise.all([
      galleryApi.getDetail(postId),
      commentApi.getComments('GALLERY', postId),
    ])
...
async function handleDelete() {
  ...
  try {
    await galleryApi.remove(postId)
...
function handleCommentCreated(newComment) { comments.value.push(newComment) }
function handleCommentDeleted(id) { comments.value = comments.value.filter(c => c.id !== id) }
...
<template>
  <div class="detail-page">
    ...
    <template v-else-if="post">
      ...
      <!-- 이미지 캐러셀 -->
      <div v-if="post.images && post.images.length" class="carousel-wrap">
      ...
      <div class="detail-actions">
        <RouterLink to="/boards/gallery" class="btn-back">목록</RouterLink>
        <template v-if="post.isEditable">
          <RouterLink :to="`/boards/gallery/modify/${post.id}`" class="btn-edit">수정</RouterLink>
      ...
      <div class="comment-section">
        ...
        <CommentInput board-type="GALLERY" :post-id="postId" @created="handleCommentCreated" />
...
.badge { display: inline-block; padding: 2px 10px; background: #fce4ec; color: #c62828; border-radius: 12px; font-size: 0.8rem; font-weight: 600; }
...
.carousel-wrap { margin-bottom: 24px; }
.detail-content {
  min-height: 100px;
  font-size: 0.95rem;
  line-height: 1.8;

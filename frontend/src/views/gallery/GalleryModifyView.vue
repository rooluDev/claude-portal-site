...
async function fetchDetail() {
  try {
    const res = await galleryApi.getDetail(route.params.id)
    const d = res.data.data
    if (!d.isEditable) {
      toastStore.show('error', '수정 권한이 없습니다.')
      router.push(`/boards/gallery/${route.params.id}`)
      return
    }
    ...
    if (status === 403) toastStore.show('error', msg || '접근 권한이 없습니다.')
    else if (status !== 401) toastStore.show('error', msg || '불러오기 실패')
...
async function handleSubmit() {
  ...
  try {
    ...
    await galleryApi.update(route.params.id, formData)
...
<template>
  <div class="write-page">
    <h1 class="write-title">갤러리 수정</h1>
    ...
    <div class="form">
      ...
      <div class="form-actions">
        <RouterLink :to="`/boards/gallery/${route.params.id}`" class="btn-cancel">취소</RouterLink>

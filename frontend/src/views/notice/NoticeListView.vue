...
function handlePageChange(page) {
  fetchList({ ...currentParams, pageNum: page })
}
...
<template>
  <div class="board-page">
    <div class="board-header">
      ...
      <RouterLink v-if="authStore.isAdmin" to="/boards/notice/write" class="btn-write">글쓰기</RouterLink>
    </div>
    ...
    <table v-else class="board-table">
      ...
      <tbody>
        ...
        <tr
          v-for="post in posts"
          :key="post.id"
          :class="{ 'pinned-row': post.isPinned }"
          @click="router.push(`/boards/notice/${post.id}`)"
          style="cursor:pointer"
        >
          <td class="col-num">{{ post.isPinned ? '-' : post.id }}</td>
